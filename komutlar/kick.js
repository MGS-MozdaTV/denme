...
let phobos = message.mentions.users.first();
...
if (!message.guild.member(phobos).bannable) return message.channel.send(`**Belirttiğiniz kişinin Yetkisi Benden Daha Üstün!**`);
message.guild.member(phobos).kick();
message.channel.send(" **Başarılı, "+ phobos +" İD Li Kullanıcı " + reason + " sebebiyle sunucudan atıldı.**")
