...
exports.run = async(client, message, args) => {
  ...
  let prefix = await require('quick.db').fetch(`prefix_${message.guild.id}`) || ayarlar.TimsahTimPrefix
...
if (!args[0])  {
    ...
    .setTitle('MGS-MozdaTV')
    .setDescription("**Reklam Korumayı Kullanmak için `reklam-engelle aç veya kapat` Yazınız**")
    .setColor('')
  ...
  if (args [0] == 'kapat') {
   ...
   const küfürengelcim22 = new Discord.MessageEmbed()
    .setTitle('')
    .setDescription('**Reklam Engelleme Sistemi deaktif edildi**')
    .setColor('GREEN')
